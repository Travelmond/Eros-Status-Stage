...
function DescField({ emoji, label, value }: { emoji: string; label: string; value?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="w-20 flex-shrink-0" style={{ color: 'var(--neon-gold)70' }}>
        {label}:
      </span>
  ...
  if (visibleSections.length === 0) {
    return (
      <div
        ...
        style={{ border: '1px solid var(--neon-gold)20', background: '#0A0A0A' }}
      >
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: 'var(--neon-gold)08', borderBottom: '1px solid var(--neon-gold)20' }}
      >
      ...
      <div style={{ background: '#0A0A0A' }}>
        {visibleSections.map((section) => {
          ...
          return (
            <div key={section.key} style={{ borderBottom: '1px solid var(--neon-gold)10' }}>
              <div
                className="flex items-center justify-between px-3 py-1.5 cursor-pointer hover:bg-white/5 transition-colors"
                onClick={() => toggleSection(section.key)}
                style={{ background: isOpen ? 'var(--neon-gold)08' : 'transparent' }}
              >
                ...
                <span className="text-xs font-mono" style={{ color: 'var(--neon-gold)40' }}>
                  {isOpen ? '▲' : '▼'}
                </span>
              </div>
              {isOpen && (

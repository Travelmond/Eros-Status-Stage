...
42. **Montagem correta do Stage — ReactRunner vs TestRunner (2026-08-15)** — `src/main.tsx` seleciona entre `App` (TestRunner, usado em dev OU com `?test` na URL) e `ReactRunner` (produção, monta o Stage dentro do iframe do Chub). `ReactRunner` não é envolvido em `StrictMode` (evita dupla montagem); `StrictMode` aplicado somente ao `App`. Adicionado `src/vite-env.d.ts` (`/// <reference types="vite/client" />`) para tipar `import.meta.env.DEV`.
43. **Busca dinâmica de TODOS os modelos do OpenRouter (2026-08-15)** — `src/services/openRouter.ts` ganhou `OPENROUTER_MODELS_URL`, `OpenRouterModelInfo` e `fetchOpenRouterModels()` (GET `/api/v1/models`, endpoint público sem API key; retorna `[]` em erro). Cache em memória (`resetOpenRouterModelsCache`) evita refetch a cada montagem; `AVAILABLE_MODELS` permanece como fallback offline.
44. **Teste de integração do ciclo de vida do Stage (2026-08-15)** — `src/Stage.test.tsx` instancia `Stage` com `InitialData` mock (`environment: 'development'`) e valida `load`, `beforePrompt` (`stageDirections`), `afterResponse` (marcadores `[💕90% 🔥60%]`), `setState` e `render`. Adicionado `sass-embedded` como devDependency (o import do `ReactRunner` puxa o `index.scss` do `@chub-ai/stages-ts`). Suite: 45 testes passando.
45. **Painel de IA dinâmico — listagem de todos os modelos OpenRouter (2026-08-15)** — `AIProviderSection.tsx` busca `fetchOpenRouterModels()` no `mount` e renderiza cartões com `name`/`description`/`context_length`/preço por milhão (tokens × 1_000_000). Estados `loading`/`error`/empty tratados (fallback `AVAILABLE_MODELS`); busca case-insensitive por `name`/`id`; opção "Use custom model" para IDs/aliases não listados; fonte de verdade permanece `config.openRouterModel`. Acessibilidade com `role="status"`/`aria-live`/`aria-pressed`.
46. **Fix de merge de config + persistência de preferências no TestRunner (2026-08-15)** — `App.tsx`: `onConfigChange={setConfig}` substituído por `handleConfigChange` que faz merge (`{ ...prev, ...patch }`), corrigindo o bug onde `openRouterApiKey` e `openRouterModel` se apagavam mutuamente. Adicionada persistência leve via `setPreference`/`getPreference` (chave `ui_config`) com sanitização (`sanitizeConfigForStorage`) que **remove `openRouterApiKey` antes de gravar** — a API key permanece apenas em memória durante a sessão. Estado inicializado mesclando `DEMO_CONFIG` com as preferências salvas (sem restaurar API key). Modelo, toggles (NTR/auditor/imgAuditor) e preferências de UI (tema, barStyle, density) sobrevivem a reload. Validação: typecheck, lint, build e 45 testes passando.
47. **Opt-in "lembrar chave nesta máquina" + validação de formato da API key (2026-08-15)** — Adicionado `rememberApiKey?: boolean` a `ConfigType` e ao `config_schema` de `chub_meta.yaml` (default `false`). A API key do OpenRouter continua NUNCA persistida por padrão; apenas quando `rememberApiKey === true` ela é gravada em uma chave dedicada (`eros_remembered_openrouter_key` em localStorage) e restaurada na inicialização do `config`. Em `App.tsx`, `handleConfigChange` grava/remove essa chave dedicada conforme `next.rememberApiKey` e `next.openRouterApiKey` (tudo em try/catch); `sanitizeConfigForStorage` continua excluindo `openRouterApiKey` das preferências normais (`ui_config`). Em `AIProviderSection.tsx`: Switch "Lembrar chave nesta máquina" (com aviso "⚠️ armazena em localStorage deste navegador"), validação visual não-bloqueante de formato (`sk-or-`) em `var(--neon-pink)` e botão 👁️/🙈 para mostrar/ocultar a chave (`type` password↔text). Validação: typecheck, lint, build e 45 testes passando.
